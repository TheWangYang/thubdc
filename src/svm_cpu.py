...
def processing_feature(file):
    ...
    if os.path.exists(f"/data1/wyy/projects/_competition/thubdc/inputs/log/{file}_log.csv"):
        log = pd.read_csv(f"/data1/wyy/projects/_competition/thubdc/inputs/log/{file}_log.csv").sort_values(
    ...
    if os.path.exists(f"/data1/wyy/projects/_competition/thubdc/inputs/trace/{file}_trace.csv"):
        trace = pd.read_csv(f"/data1/wyy/projects/_competition/thubdc/inputs/trace/{file}_trace.csv").sort_values(
            by=['timestamp']).reset_index(drop=True)

    if os.path.exists(f"/data1/wyy/projects/_competition/thubdc/inputs/metric/{file}_metric.csv"):
        metric = pd.read_csv(f"/data1/wyy/projects/_competition/thubdc/inputs/metric/{file}_metric.csv").sort_values(
            by=['timestamp']).reset_index(drop=True)
...
all_data = feature.merge(label[['id', 'label']].groupby(['id'], as_index=False)['label'].agg(list),
                         how='left', on=['id']).set_index("id")
...
# 构造文件名
file_name = f"svm_{current_time}.csv"
