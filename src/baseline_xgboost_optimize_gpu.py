...
print("all_data.columns: {}".format(all_data.columns))

# 设置不使用的列名
not_use = ['id', 'label', 'trace_endpoint_name_nunique', 'trace_host_ip_nunique',
           'trace_service_name_nunique', 'trace_status_code_std', 'metric_length']
...
#     'max_depth': 4,
#     'subsample': 0.8,
#     'alpha': 0.1,  # 设置 alpha 参数
#     'lambda': 0.1,  # 设置 lambda 参数
...
# 构造文件名
file_name = f"baseline_xgboost_optimize_gpu_{current_time}.csv"
...
# # 将输出内容写入CSV文件
# with open("../logs/baseline_xgboost_optimize_gpu.csv", "w", newline="") as f:
#     writer = csv.writer(f)
