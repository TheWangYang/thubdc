...
for train_index, valid_index in kf.split(train_scaler_X, y):
    ...
    # Replace XGBClassifier with CNN
    model = Sequential()
    model.add(Conv1D(filters=64, kernel_size=3, activation='relu',
              input_shape=(len(feature_name), 1)))
    model.add(Conv1D(filters=64, kernel_size=3, activation='relu'))
    ...
    model.fit(X_train.reshape(
        X_train.shape[0], X_train.shape[1], 1), y_train, epochs=10, batch_size=64, verbose=1)

    ovr_oof[valid_index] = model.predict(
        X_valid.reshape(X_valid.shape[0], X_valid.shape[1], 1))
    ovr_preds += model.predict(test_scaler_X.reshape(
        test_scaler_X.shape[0], test_scaler_X.shape[1], 1)) / n_splits
...
# 构造文件名
file_name = f"cnn_{current_time}.csv"
