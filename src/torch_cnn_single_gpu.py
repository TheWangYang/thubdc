...
X_train_notensor, X_val_notensor, y_train_notensor, y_val_notensor = train_test_split(
    train_scaler_X, y, test_size=0.2, random_state=3407)
...
# 初始化神经网络和优化器，并将它们移到CUDA设备上
net = DeepNet(input_dim, hidden_dim, output_dim).to(device)
criterion = nn.BCEWithLogitsLoss()
optimizer = optim.Adam(net.parameters(), lr=learning_rate)

# 得到train_dataloader
X_train = torch.tensor(X_train_notensor, dtype=torch.float32)
y_train = torch.tensor(y_train_notensor, dtype=torch.float32)
train_dataset = CustomDataset(X_train, y_train)
train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)

# 得到val_dataloader
X_val = torch.tensor(X_val_notensor, dtype=torch.float32)
y_val = torch.tensor(y_val_notensor, dtype=torch.float32)
...
for epoch in range(num_epochs):
    ...
    for inputs, labels in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
        inputs = inputs.to(device)  # 将输入数据移到CUDA设备上
        labels = labels.to(device)  # 将标签移到CUDA设备上
        optimizer.zero_grad()
        outputs = net(inputs)
...
net.eval()
ovr_oof = np.zeros((len(X_val_notensor), num_classes))
with torch.no_grad():
...
# Calculate scores
each_score = sScore(y_val_notensor, ovr_oof)
...
net.eval()
with torch.no_grad():
    for i, inputs in enumerate(tqdm(test_loader, desc="Test")):
        inputs = inputs.to(device)  # 将输入数据移到CUDA设备上
        outputs = net(inputs)
...
file_name = f"torch_cnn_single_gpu_{current_time}.csv"
